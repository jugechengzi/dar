...
import model
from beer import BeerData, BeerAnnotation, BeerData1
...
from train_util import train_teacher,train_share,train_skew
...
sentence_data = BeerData1(args.data_dir, args.aspect, 'train', word2idx, balance=True)
...
sentence_loader = DataLoader(sentence_data, batch_size=500, shuffle=True)
...
def pre_tea(epochs):
    ...
    for e in range(epochs):
        ...
        if best_dev<dev_f1:
            best_dev=dev_f1
            best_e=e
            torch.save(tea_model,'./save_model/beer/tea_model_{}.pth'.format(args.aspect))
    print('pre_acc={},epoch={}'.format(best_dev,best_e))

if args.pre_epoch>0:
    pre_tea(args.pre_epoch)
tea_model=torch.load('./save_model/beer/tea_model_{}.pth'.format(args.aspect)).to(device).train()
...
###skew train
skew_optimizer = torch.optim.Adam(model.get_cls_param(), lr=0.001)
for e in range(args.skew):
    model.train()
    precision, recall, f1_score, accuracy = train_skew(model, skew_optimizer, sentence_loader, device, args)
    print('skew={},p={:.4f},r={:.4f},acc={:.4f}'.format(e, precision, recall, accuracy))
